...
public class Inflector {
    ...
    private Inflector() {
        ...
        addPlural("(.*)(hive)$", "\\1\\2s");
        addPlural("(.*)(tive)$",
                  "\\1\\2s"); // Added for consistency with singular rules
        addPlural("(.*)([^aeiouy]|qu)y$", "\\1\\2ies");
        addPlural("(.*)(series)$",
                  "\\1\\2"); // Added for consistency with singular rules
        addPlural("(.*)(movie)$",
                  "\\1\\2s"); // Added for consistency with singular rules
        addPlural("(.*)(x|ch|ss|sh)$", "\\1\\2es");
        addPlural("(.*)(matr|vert|ind)ix|ex$", "\\1\\2ices");
        addPlural("(.*)(o)$",
                  "\\1\\2es"); // Added for consistency with singular rules
        addPlural("(.*)(shoe)$",
                  "\\1\\2s"); // Added for consistency with singular rules
        addPlural("(.*)([m|l])ouse$", "\\1\\2ice");
        addPlural("^(ox)$", "\\1en");
        addPlural("(.*)(vert|ind)ex$",
                  "\\1\\2ices"); // Added for consistency with singular rules
        addPlural("(.*)(matr)ix$",
                  "\\1\\2ices"); // Added for consistency with singular rules
        ...
        addSingular("(.*)((a)naly|(b)a|(d)iagno|(p)arenthe|(p)rogno|(s)ynop|(t)he)ses$",
                    "\\1\\2sis");
    ...
    public void addIrregular(String singular, String plural) {
        ...
        addPlural("(.*)(" + singular.substring(0, 1) + ")" +
                  singular.substring(1) + "$", "\\1\\2" + plural.substring(1));
        addSingular("(.*)(" + plural.substring(0, 1) + ")" +
                    plural.substring(1) + "$",
    ...
    private class Replacer {
        ...
        public Replacer(String match, String rule, boolean insensitive) {
            ...
            pattern =
                    Pattern.compile(match, insensitive ? Pattern.CASE_INSENSITIVE :
                                           0);
            this.rule = rule;
    ...
    public static void main(String[] args) {
        String[] words =
            new String[] { "beeIds", "matches", "announcements", "currentActivities" };
        Inflector ifl = new Inflector();
        for (int i = 0; i < words.length; i++) {
            System.out.println("Singular of " + words[i] + " is " +
                               ifl.singularize(words[i]));
        }
    }
}
